...
pub struct Executor {
    ...
    is_directed: bool,
    //aibar added a variable for counting number of executions of binary
    pub localCounter: &'static mut u64,
    //variables for Dung
    pub toAddFuncName: bool,
    pub funcName: String, 
}
...
impl Executor {
    pub fn new(
        ...
        //aibar added
        static mut temp_localCounter: u64 = 0;
        unsafe{ Self {
            cmd,
            branches,
            t_conds,
            ...
            is_directed,
            localCounter: &mut temp_localCounter,
            toAddFuncName: false,
            funcName: "".to_owned(),
        }}
    }
    ...
    fn try_unlimited_memory(&mut self, buf: &Vec<u8>, cmpid: u32) -> bool {
        ...
        //aibar added: here it will possible execute a binary
        *self.localCounter+=1;
        let unmem_status =
    ...
    fn write_test(&mut self, buf: &Vec<u8>) {
        //Uncomment to add fname to the input for DUNG
        //It adds func name to the beginning of input file
        /*
        if(self.toAddFuncName)
        {
          let mut bufModified = buf.to_vec();
          let mut name = self.funcName.clone();
          let mut i = 0;
          for c in name.chars(){
              bufModified.insert(i, c as u8);  
              i = i + 1;
          }
          self.fd.write_buf(&bufModified);  
        }
        else
        {
        ...
        */
        //if above lines are commneted then uncomment the lower line!!!!
        //ans vice versa 
        self.fd.write_buf(buf);
        if self.cmd.is_stdin {
            self.fd.rewind();
