...
def build_pipeline(bc_file, target_flags="@@", profiling_input_dir="in", is_cpp=False):
    ...
    #checkThings = "-Wl, -allow-multiple-definition -Wl,--whole-archive /parmesan/bin/lib/libdungn_rt-x86_64.a -Wl,--no-whole-archive -Wl,--dynamic-list=/parmesan/bin/lib/libdungn_rt-x86_64.a.syms -lstdc++ -lrt"
    #int2kThings = "-Wl, -allow-multiple-definition -Xclang -load -Xclang /parmesan/bin/pass/libDungnPass.so  -Wl,--whole-archive /parmesan/bin/lib/libdungn_rt-x86_64.a -Wl,--no-whole-archive -Wl,--dynamic-list=/parmesan/bin/lib/libdungn_rt-x86_64.a.syms -lstdc++ -lrt"
  
   # int2kThings_old = "-Wl, -allow-multiple-definition -Xclang -load -Xclang /parmesan/bin/pass/libDungnPass.so -pie -fpic -Qunused-arguments -g -O3 -funroll-loops -Wl,--whole-archive /parmesan/bin/lib/libdungn_rt-x86_64.a -Wl,--no-whole-archive -Wl,--dynamic-list=/parmesan/bin/lib/libdungn_rt-x86_64.a.syms -lstdc++ -lrt -Wl,--no-as-needed -Wl,--gc-sections -ldl -lpthread -lm -Xclang -load -Xclang /parmesan/bin/pass/libUnfoldBranchPass.so -Xclang -load -Xclang /parmesan/bin/pass/libAngoraPass.so -mllvm -angora-dfsan-abilist=/parmesan/bin/rules/angora_abilist.txt -mllvm -angora-dfsan-abilist=/parmesan/bin/rules/dfsan_abilist.txt -mllvm -angora-exploitation-list=/parmesan/bin/rules/exploitation_list.txt -pie -fpic -Qunused-arguments -g -O3 -funroll-loops /parmesan/bin/lib/libruntime_fast.a -Wl,--no-as-needed -Wl,--gc-sections -ldl -lpthread -lm"
    aibarThings = "-Wl,--allow-multiple-definition -Wl, -Xclang -load -Xclang /parmesan/bin/pass/libAibarPass.so  -Wl,--whole-archive /parmesan/bin/lib/libaibar_rt-x86_64.a -Wl,--no-whole-archive -Wl,--dynamic-list=/parmesan/bin/lib/libaibar_rt-x86_64.a.syms -lstdc++ -lrt"
    #1) BUILD FAST LL
    run_cmd(f"USE_FAST=1 {compiler} {cflags} -S -emit-llvm -o {name}.fast.ll  {bc_file}")
    #2) BUILD FAST BIN
    run_cmd(f"USE_FAST=1 {compiler} {cflags} {aibarThings} -o {name}.fast {bc_file} ")
    #3) BUILD FAST SAN LL
    run_cmd(f"USE_FAST=1 {compiler} {cflags} -fsanitize={sanitizer} -S -emit-llvm -o {name}.san.ll  {bc_file} ")
    #4) BUILD FAST SAN BIN
    run_cmd(f"USE_FAST=1 {compiler} {cflags} {aibarThings} -fsanitize={sanitizer}  -o {name}.san.fast  {bc_file}")
    #5) BUILD TRACK BIN
